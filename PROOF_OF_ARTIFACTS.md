...
| [TIMELINE_OF_INVENTION.md](TIMELINE_OF_INVENTION.md) | Dated, commit-linked record with human/AI attribution per entry |
...
| [claude-skill/](claude-skill/) | Claude Code skill (offline) that drives the binary |
| src/ | Rust validator binary (`f30`) and TRIPLE SIMS test gate |

## Claude Code Skill

The [`claude-skill/`](claude-skill/) directory ships an offline Claude Code skill
that drives the `provenance-docs` binary. With it installed, Claude will
auto-stub TOI/POA entries after commits, draft AI Role attribution from the
diff, and run `f30` before releases.

```bash
# Install the binary (one-time)
cargo install --path .

# Install the skill (one-time, per machine)
mkdir -p ~/.claude/skills/provenance-docs
cp claude-skill/SKILL.md ~/.claude/skills/provenance-docs/SKILL.md
```

Restart Claude Code. The skill activates automatically in repos containing
`TIMELINE_OF_INVENTION.md` and `PROOF_OF_ARTIFACTS.md`. No network calls, no
telemetry — the skill is pure markdown that triggers the local Rust binary.
